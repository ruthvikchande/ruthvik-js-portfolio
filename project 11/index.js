...
const datacointainer=document.querySelector("#data");
// console.log(datacointainer);
const generate=()=>{
    console.log(item.value);
    if(isNaN(item.value) || item.value < 0 || item.value > 9){
        const randomindex=Math.floor(Math.random() * text.length);
datacointainer.innerHTML=`<p>${text[randomindex]}</p>`
    }else{
const data=text.slice(0,item.value);
const para=data.map((d)=>{
    return `<p>${d}</p>`
})
datacointainer.innerHTML=para.join()    }
